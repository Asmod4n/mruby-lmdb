...
mrb_lmdb_fix2bin(mrb_state *mrb, mrb_int number)
{
  ...
  p[3] = (uint8_t)(number >> 32);
  p[4] = (uint8_t)(number >> 24);
...
/* ========================================================================
 * MDB bulk operations (no Ruby re-entry between items)
 * ======================================================================== */

/*
 * MDB.multi_get(txn, dbi, keys_array) -> array of values (or nil per miss)
 *
 * Single RDONLY txn, N mdb_get calls, no Ruby method dispatch in the loop.
 */
static mrb_value
...
/*
 * MDB.batch_put(txn, dbi, pairs_array, flags=0)
 *
 * pairs_array is [[key, val], [key, val], ...]
 * Single write txn, N mdb_put calls, no Ruby method dispatch in the loop.
 */
static mrb_value
